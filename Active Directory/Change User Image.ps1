...
Do
{
	...
	if ($width -gt $height)
	{
		...
		$imageLg = $image | Set-ImageFilter -filter (Add-ScaleFilter -Width 648 -Height 648 -passThru) -passThru
		$imageSm = $image | Set-ImageFilter -filter (Add-ScaleFilter -Width 96 -Height 96 -passThru) -passThru
	}
	...
	$imageSm.SaveFile("$save")

	Clear-Host
	Write-Host "Applying to ADUC property..."
    $userID, $leftoverjunk = $userEmail.split('@')
	$aducphoto = [byte[]](Get-Content $save -Encoding byte)
	Set-ADUser $userID -Replace @{ thumbnailPhoto = $aducphoto } 
	Remove-Item -Path $save -force
