...
$FileOutputDirectory = "C:\Temp\UPNMatch"
...
Get-ADUser -Filter * -SearchBase $UserOU -Properties * | where {$_.EmailAddress -ne $null} | select name,givenName,Surname,UserPrincipalName,EmailAddress,SamAccountName | Export-Csv -NoTypeInformation $FileOutputDirectory\UPNMatchBackup.csv

#Gather AD Users and Start Foreach Loop
Get-ADUser -Filter * -SearchBase $UserOU -Properties * | where {$_.EmailAddress -ne $null} | foreach {
...
if ($(Test-Path C:\temp) -eq $true)
    {
    $table | Export-Csv -NoTypeInformation $FileOutputDirectory\UPNMatch.csv -Verbose
    }
else
    {
    mkdir C:\Temp
