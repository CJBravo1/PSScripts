#Change Window Title
$host.ui.RawUI.WindowTitle = "Accruent New Hire Script"

Start-Transcript -OutputDirectory ("$pwd\Transcripts") -IncludeInvocationHeader

# Acquisition Toggle
# When set to 0, run the normal User Account Creation
# When set to 1, The user will be hidden from the GAL, no manager will be entered 
$AcquisitionFlag = 0
...
foreach ($User in $Users)  
    {
	    ...
	    $TempVar = $User.Manager
        $Manager = (Get-ADUser -filter {Name -like $TempVar}).SamAccountName
        $FullName = "$FirstName $Lastname"
        $Product = $User.Product
        $OnboardDate = $User.OnboardDate
        $FirstDayLocation = $User.FirstDayLocation
        $LaptopModel = $User.laptopmodel

        if (($User.MiddleInitial -eq "") -or (($FirstName + $LastName).length -gt 17))
            {
            ...
            {
            $Username = ("$FirstName"+"."+"$MiddleInitial"+"."+"$LastName1")
            $UID = $Username.ToLower()
            $Email = $UID+"@accruent.com"
            }
    ...
    New-ADUser -Name $FullName -DisplayName $FullName -Path "OU=acc_Domain_Users,DC=accruent,DC=com" -SamAccountName $UID -GivenName $FirstName -Surname $LastName -Company $Product -AccountPassword (ConvertTo-SecureString "V@lh8lla69" -AsPlainText -Force) -ChangePasswordAtLogon $false -ScriptPath "LOGIN.CMD" -Enabled $true
    
    #Hide user from GAL if this is an acquisition account creation
    #Also forgoes setting Department and Manager properties
    if($AcquisitionFlag -eq 1)
    {
        Set-ADUser -identity $UID -replace @{'msExchHideFromAddressLists' = $True}
        Set-ADUser -identity $UID -EmailAddress $Email -userPrincipalName $Email -Description $Description -Title $Description 
    }
    else
    {
        # build command to update AD information, checking to see if variable has value, if it does, add it to the command
        # break out the command for assigning manager to a different line in case the data isn't entered properly and errors out - it will only effect that one property instead of aborting the whole line of code
        Set-ADUser -identity $UID -EmailAddress $Email -userPrincipalName $Email -Description $Description -Title $Description -Department $Department
        Set-ADUser -identity $UID -Manager $Manager -physicalDeliveryOfficeName $OfficeName
        Set-ADUser -Identity $UID -Add @{extensionattribute5="$OfficeName"}
    }
        
    #Changes Proxy and Target Address' in Attibute Editor
    Write-Host "Adding Proxy Addresses" -Foregroundcolor Green
    Set-ADUser -Identity $UID -Add @{ProxyAddresses="SMTP:$UID@accruent.com"}
    ...
    Add-ADGroupMember "HRBenefits" $UID
    Add-ADGroupMember "_ALL_ Ring The Bell" $UID
    Add-ADGroupMember "VPN_General" $UID
    #Add-ADGroupMember "_ALL_ Dropbox Access" $UID
    ...
    Write-Host "Adding User to Location based AD Groups" -Foregroundcolor Green
    Switch ($OfficeName)
    {
        ...
        "London"
            ...
            Set-ADUser -Identity $UID -StreetAddress "111 Buckingham Palace Road" -City "Victoria" -PostalCode "SW1W 0SR" -Country "United Kingdom"
        ...
        "Denver"
            {
            Set-ADUser -Identity $UID -Office "CO-DENVER"
            Set-ADUser -Identity $UID -StreetAddress "6465 Greenwood Plaza Boulevard, Suite 600" -City "Centennial" -State "CO" -PostalCode "80111" -Country "US"
            }
        "Dusseldorf"
            {
        ...
        {($_ -eq "LA") -or ($_ -eq "New Orleans") -or ($_ -eq "NOLA")}
            ...
            Set-ADUser -Identity $UID -StreetAddress "900 Camp Street, Office 317" -City "New Orleans" -State "LA" -PostalCode "70130" -Country "US"
    ...
    Switch -wildcard ($Department)
    {
        ...
        "Assessment Services*"
            {
            Add-ADGroupMember "Assessment Services" $UID
            }
        "Business Services*"
            {
            }
        ...
        {($_ -eq "Lucernex") -or ($_ -eq "LX")}
            {
                ...
                if($OfficeName -eq "Plano")
                {
                    Add-ADGroupMember "_LX_ Lx Plano" $UID
                }
            }
    }
    <#
    ...
    #>
    ############################### END SECTION ADD TO ACTIVE DIRECTORY GROUPS ###########################################
    ...
    $ConfluenceEmailBody += "$UID@accruent.com,<br>"
    ...
    else
    {
        ...
        $nonDA = ($UserCredential.UserName).replace("da_","")
        $office = Get-ADUser $UID -Properties * | Select Office 
        $GroupsBody =
        "$ManagerFirst,
        <br>
        <br>
        In preparation for $FullName's first day of work at, please
        ...
        <br> The link will create a pre-formed email that has syntax for creating a KACE ticket. You only need to list the groups at the bottom of the message and click send.
        <br><br>    
        <b>Please be specific -</b> We will not mirror another user's access, as they don't always line up 100%. Consider using the Address Book in Outlook to search for groups and check the group membership of other users.
        <br><br>
        <a href= https://accruent.atlassian.net/wiki/spaces/IT/pages/404390193/OUTLOOK+Using+the+Address+Book>How to use the address book to find group memberships</a>
        <br><br>
        Thank you,
        <br><br>
        IT Service Desk "
        ...
        Send-MailMessage -To $ManagerEmail -From $nonDA -Subject "REQUEST | New Hire Security/Distro Groups - $FullName" -Body $GroupsBody -BodyAsHtml -Credential $UserCredential -SmtpServer $mxserver -UseSsl   
    }
    
    #Create KACE ticket for setting up computer
    Write-Host "Submitting Kace Ticket" -ForegroundColor Yellow -BackgroundColor Red
    $AccTicketTo = "helpdesk1@accruent.com"
    $AccTicketSubject = "ACTION | Please set up $laptopmodel computer for $FullName in $FirstDayLocation before $OnboardDate "
    $AccTicketBody = 
    "@priority= High (3 Bus Days)
     @category= Hardware
     @location= $OfficeName
     @CUSTOM_FIELD_VALUE0= $office
     Please prepare a computer for $Fullname.
        
     Image with Windows 10, install MS Teams, setup Onedrive backup, create desktop shortcuts, and launch Outlook for the first time."
        
    # Submit ticket to KACE
    Send-MailMessage -To $AccTicketTo -Subject $AccTicketSubject -Body $AccTicketBody -SmtpServer $mxserver -From $nonDA -Credential $UserCredential -UseSsl    
...
#New Hire Created Email
Send-MailMessage -To "servicedesk@accruent.com" -From $Sender -Subject "INFO | New Hire Created (forward to NewHireCreated DL)" -Body $EmailBody -BodyAsHtml -Credential $UserCredential -SmtpServer $mxserver -UseSsl

#Reminder to send confluence invites (ideally just copy and paste this body into the Confluence 'Invite Users' box)
Send-MailMessage -To "servicedesk@accruent.com" -From $Sender -Subject "ACTION | Send Confluence Invites to These People" -Body $ConfluenceEmailBody -BodyAsHtml -Credential $UserCredential -SmtpServer $mxserver -UseSsl
