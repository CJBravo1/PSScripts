...
$PSSession = Get-PSSession | Where-Object {$_.ComputerName -like "*ADDS*" -or $_.ComputerName -like "SUROROFDC0*"}
...
{
    $ExchangeServer = Read-Host -Prompt "Enter Exchange Server"
    $exchange = New-PSSession -ConfigurationName Microsoft.exchange -ConnectionUri "http://$ExchangeServer/powershell" -Credential $adminCreds
...
if ($null -ne $ADuser)
{
    ...
    $primarySMTPAddress = $ADUserinput+"@$DomainName.com"
    $targetAddress = $ADUserinput+"@$DomainName.onmicrosoft.com"
    $targetAddress = "SMTP:$targetAddress"
    $ADUserEmail = $ADUser.EmailAddress
    $PrimaryProxy = "SMTP:$ADUserEmail"
    $SecondaryProxy = "smtp:$ADUserinput@$DomainName.mail.onmicrosoft.com"
