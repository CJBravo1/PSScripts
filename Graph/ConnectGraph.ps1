    ...
    if ($null -eq $MGContext)
    {
       $Write =  Read-Host "Read or Write?"
        switch ($write)
        {
            "Write"
            {
                Connect-MgGraph -Scopes 'Directory.ReadWrite.All','User.ReadWrite.All','Group.ReadWrite.All','User.Export.All','User.Read.All','Directory.AccessAsUser.All','User.ReadBasic.All','Directory.Read.All'
            }
            "Read"
            {
                Connect-MgGraph -Scopes 'User.Read.All', 'Directory.AccessAsUser.All', 'User.ReadBasic.All', 'User.ReadWrite.All', 'Directory.Read.All', 'Directory.ReadWrite.All', 'Group.Read.All', 'User.Export.All'
            }
