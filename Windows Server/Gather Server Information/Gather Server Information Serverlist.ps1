...
$os = (Get-WmiObject -ComputerName $server Win32_OperatingSystem).Caption
...
$processor = (Get-WmiObject -ComputerName $server Win32_Processor).Caption

# Get RAM information
Write-Green "Gathering RAM information..."
$ram = (Get-WmiObject -ComputerName $server Win32_ComputerSystem).TotalPhysicalMemory
$ramGB = [Math]::Round($ram / 1GB, 2)

# Get RAM usage information
Write-Green "Gathering RAM usage information..."
$ramUsage = (Get-WmiObject -ComputerName $server Win32_OperatingSystem).TotalVisibleMemorySize
$ramUsageGB = [Math]::Round($ramUsage / 1GB, 2)

# Get installed applications
Write-Green "Gathering installed applications..."
$applications = Get-WmiObject -ComputerName $server win32_product | 
...
$networkInterfaces = Invoke-Command -ComputerName $server -ScriptBlock { Get-NetAdapter | Where-Object { $_.Status -eq "Up" } |
                     ForEach-Object {
...
$drives = Get-WmiObject -ComputerName $server Win32_LogicalDisk | Where-Object { $_.DriveType -eq 3 } | Select-Object DeviceID, VolumeName, Size, FreeSpace
...
$drivers = Get-WmiObject -ComputerName $server Win32_PnPSignedDriver | Where-Object { $_.DeviceName -ne $null } |
...
$networkShares = Get-WmiObject -ComputerName $server Win32_Share | Select-Object Name, Path
...
$sharedPrinters = Get-WmiObject -ComputerName $server Win32_Printer | Where-Object { $_.Shared -eq $true } |
...
$serverExportFileName = "${server}_ServerInfo.csv"
...
$networkInterfacesExportFileName = "${server}_NetworkInterfaces.csv"
...
$driversExportFileName = "${server}_Drivers.csv"
